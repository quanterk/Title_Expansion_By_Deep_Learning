...
class ParallelDataset(data.Dataset):
    ...
    @staticmethod
    def sort_key(ex):
        return data.interleave_keys(len(ex.src), len(ex.trg_index))

    def __init__(self, src_examples, trg_index_examples, trg_socre_example, fields, **kwargs):
        ...
        if not isinstance(fields[0], (tuple, list)):
            if trg_index_examples is None:
                fields = [('src', fields[0])]
            else:
                fields = [('src', fields[0]), ('trg_index', fields[1]), ('trg_score', fields[2])]
        ...
        if trg_index_examples is None:
            ...
            for src_line, trg_index_line, trg_score_line in zip(src_examples, trg_index_examples, trg_socre_example):
                examples.append(data.Example.fromlist(
                    [src_line, trg_index_line, trg_score_line], fields))

        super(ParallelDataset, self).__init__(examples, fields, **kwargs)


class ParallelDatasetOriginal(data.Dataset):
    """Defines a custom dataset for machine translation."""

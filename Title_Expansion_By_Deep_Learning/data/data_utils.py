...
from data.dataset import ParallelDatasetOriginal
import torch
...
def load_train_data(data_path, batch_size, max_src_len, max_trg_len, use_cuda=False):
    ...
    train_data = ParallelDatasetOriginal(train_src, train_tgt, fields=fields, filter_pred=filter_pred,)
    ...
    dev_data = ParallelDatasetOriginal(dev_src, dev_tgt, fields=fields,)
...
def load_test_data(data_path, vocab_path, batch_size, use_cuda=False):
    ...
    test_data = ParallelDatasetOriginal(test_src, None, fields=fields,)
...
def norm(x):
    ...
    total_sum = np.sum(x, axis=1).reshape(-1, 1)

    return x / (total_sum + 0.0001)
...
def all_chinese(string):
    for i in range(len(string)):
        if not is_chinese_word(string[i]):
            return False
    return True
